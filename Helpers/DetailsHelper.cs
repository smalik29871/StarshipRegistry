...
namespace StarshipRegistry.Helpers
{
    public class DetailsHelper
    {
        ...
        public string GetFilmTitle(string url)
        {
            var film = _context.Films.FirstOrDefault(f => f.Url == url);
            return film?.Title ?? "Unknown Film";
        }

        public async Task<string> GetFilmTitleAsync(string url)
        {
            var film = _context.Films.FirstOrDefault(f => f.Url == url);
            if (film != null)
                return film.Title;

            var fetchedFilm = await _swapiService.FetchFromApiAsync<Film>(url);
            if (fetchedFilm != null)
            {
                try
                {
                    _context.Films.Add(fetchedFilm);
                    await _context.SaveChangesAsync();
                    return fetchedFilm.Title;
                }
                catch (DbUpdateException)
                {
                    return fetchedFilm.Title ?? "Unknown Film";
                }
            }
        ...
        public async Task<string> GetCharacterNameAsync(string url)
        {
            var character = _context.Characters.FirstOrDefault(c => c.Url == url);
            if (character != null)
                return character.Name;

            var fetchedCharacter = await _swapiService.FetchFromApiAsync<Character>(url);
            if (fetchedCharacter != null)
            {
                try
                {
                    _context.Characters.Add(fetchedCharacter);
                    await _context.SaveChangesAsync();
                    return fetchedCharacter.Name;
                }
                catch (DbUpdateException)
                {
                    return fetchedCharacter.Name ?? "Unknown Pilot";
                }
            }
        ...
        public async Task<string> GetPlanetNameAsync(string url)
        {
            var planet = _context.Planets.FirstOrDefault(p => p.Url == url);
            if (planet != null)
                return planet.Name;

            var fetchedPlanet = await _swapiService.FetchFromApiAsync<Planet>(url);
            if (fetchedPlanet != null)
            {
                try
                {
                    _context.Planets.Add(fetchedPlanet);
                    await _context.SaveChangesAsync();
                    return fetchedPlanet.Name;
                }
                catch (DbUpdateException)
                {
                    return fetchedPlanet.Name ?? "Unknown Planet";
                }
            }
        ...
        public async Task<string> GetStarshipNameAsync(string url)
        {
            var starship = _context.Starships.FirstOrDefault(s => s.Url == url);
            if (starship != null)
                return starship.Name;

            var fetchedStarship = await _swapiService.FetchFromApiAsync<Starship>(url);
            if (fetchedStarship != null)
            {
                try
                {
                    _context.Starships.Add(fetchedStarship);
                    await _context.SaveChangesAsync();
                    return fetchedStarship.Name;
                }
                catch (DbUpdateException)
                {
                    return fetchedStarship.Name ?? "Unknown Starship";
                }
            }
        ...
        public async Task<string> GetVehicleNameAsync(string url)
        {
            var vehicle = _context.Vehicles.FirstOrDefault(v => v.Url == url);
            if (vehicle != null)
                return vehicle.Name;

            var fetchedVehicle = await _swapiService.FetchFromApiAsync<Vehicle>(url);
            if (fetchedVehicle != null)
            {
                try
                {
                    _context.Vehicles.Add(fetchedVehicle);
                    await _context.SaveChangesAsync();
                    return fetchedVehicle.Name;
                }
                catch (DbUpdateException)
                {
                    return fetchedVehicle.Name ?? "Unknown Vehicle";
                }
            }
        ...
        public async Task<string> GetSpeciesNameAsync(string url)
        {
            var species = _context.Species.FirstOrDefault(s => s.Url == url);
            if (species != null)
                return species.Name;

            var fetchedSpecies = await _swapiService.FetchFromApiAsync<Species>(url);
            if (fetchedSpecies != null)
            {
                try
                {
                    _context.Species.Add(fetchedSpecies);
                    await _context.SaveChangesAsync();
                    return fetchedSpecies.Name;
                }
                catch (DbUpdateException)
                {
                    return fetchedSpecies.Name ?? "Unknown Species";
                }
            }
